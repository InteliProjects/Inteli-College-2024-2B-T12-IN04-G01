...
public:
    ...
    // Método para detectar vibração usando o acelerômetro
    bool detectVibration() {
        ...
        if (Wire.available() == 7) {             // Verifica se todos os 7 bytes foram recebidos
            ...
            return false;                        // Retorna 'false' indicando que a vibração não foi detectada corretamente
        }
        ...
        Serial.println(magnitude < lowVibrationThreshold || magnitude > highVibrationThreshold); // Exibe 'true' ou 'false' baseado no limiar

        // Retorna 'true' se a magnitude estiver fora da faixa normal (indicando vibração), caso contrário 'false'
        return (magnitude < lowVibrationThreshold || magnitude > highVibrationThreshold);
    }
};

// Variáveis globais e instâncias
LEDManager leds(LED_RED, LED_GREEN);             // Instância da classe LEDManager para controlar os LEDs
...
void setup() {
    ...
    if (!bme.begin(BME280_ADDRESS)) {              // Tenta iniciar o sensor BME280
...
void checkTemperatureAndVibration() {
    bool isVibrating = sensors.detectVibration();    // Verifica se há vibração detectada pelo acelerômetro
